...
class DistanceMSELoss(nn.Module):
    ...
    def __init__(self, ):
        super(DistanceMSELoss, self).__init__()

    def forward(self, prediction, tgt, mask, eps=1e-6, reduction='mean'):
        tgt = torch.masked_select(tgt, mask)
        predictions = torch.masked_select(prediction, mask)
        prediction = 1 / ((prediction ** 2) + eps)
        tgt = 1 / ((tgt ** 2) + eps)
        return F.mse_loss(prediction, tgt, reduction=reduction)
